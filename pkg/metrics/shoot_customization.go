...
const metricShootsCustomPrefix = "garden_shoot_custom"

var shootCustomizationMetrics = []*template.MetricTemplate{
	// // General customization.
	{
		Name:   fmt.Sprintf("%s_privileged_containers_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_extensions_sum", metricShootsCustomPrefix),
		Help:   "Count of Shoots which have an extension configured.",
	...
	{
		Name:   fmt.Sprintf("%s_apiserver_basicauth_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_apiserver_auditpolicy_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_apiserver_oidcconfig_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_apiserver_featuregates_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_apiserver_admisionplugins_sum", metricShootsCustomPrefix),
		Help:   "Count of Shoots with enabled kube apiserver feature gates",
		Labels: []string{"admisionplugin"},
		Type:   template.Gauge,
		CollectFunc: func(obj interface{}, params ...interface{}) (*[]float64, *[][]string, error) {
			shoots, ok := obj.([]*gardenv1alpha1.Shoot)
			if !ok {
			...
			var admisionPluginCounters = map[string]float64{}
			for _, s := range shoots {
				if s.Spec.Kubernetes.KubeAPIServer != nil {
					for _, admissionPlugin := range s.Spec.Kubernetes.KubeAPIServer.AdmissionPlugins {
						// TODO Check if this determination is correct
						if admissionPlugin.Config != nil {
							admisionPluginCounters[admissionPlugin.Name]++
						}
					}
				}
			}
			var (
				timeSeriesCount = len(admisionPluginCounters)
				values          = make([]float64, 0, timeSeriesCount)
				labelSets       = make([][]string, 0, timeSeriesCount)
			)
			for i, k := range admisionPluginCounters {
				labelSets = append(labelSets, []string{i})
				values = append(values, k)
			}
			return &values, &labelSets, nil
	...
	{
		Name:   fmt.Sprintf("%s_kcm_nodecidrmasksize_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_kcm_horizontalpodautoscale_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_kcm_featuregates_sum", metricShootsCustomPrefix),
		Help:   "Sum of Shoots with enabled kube controller manager feature gates",
		Labels: []string{"featuregate"},
		Type:   template.Gauge,
		CollectFunc: func(obj interface{}, params ...interface{}) (*[]float64, *[][]string, error) {
			shoots, ok := obj.([]*gardenv1alpha1.Shoot)
	...
	{
		Name:   fmt.Sprintf("%s_scheduler_featuregates_sum", metricShootsCustomPrefix),
		Help:   "Sum of Shoots with enabled kube scheduler feature gates",
		Labels: []string{"featuregate"},
		Type:   template.Gauge,
		CollectFunc: func(obj interface{}, params ...interface{}) (*[]float64, *[][]string, error) {
			shoots, ok := obj.([]*gardenv1alpha1.Shoot)
	...
	{
		Name:   fmt.Sprintf("%s_kubelet_podpidlimit_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_proxy_mode_sum", metricShootsCustomPrefix),
		Help:   "Count of Shoots which have a proxy mode configuration for the kube proxy.",
		Labels: []string{"mode"},
		Type:   template.Gauge,
		CollectFunc: func(obj interface{}, params ...interface{}) (*[]float64, *[][]string, error) {
			shoots, ok := obj.([]*gardenv1alpha1.Shoot)
	...
	{
		Name:   fmt.Sprintf("%s_worker_multiplepools_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_worker_multizones_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_worker_taints_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_worker_labels_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_worker_annotations_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_network_customdomain_sum", metricShootsCustomPrefix),
	...
	// Hibernation customization.
	{
		Name:   fmt.Sprintf("%s_hibernation_enabled_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_hibernation_schedule_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_maintenance_window_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_maintenance_autoupdate_k8sversion_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_maintenance_autoupdate_imageversion_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_addon_nginxingress_sum", metricShootsCustomPrefix),
	...
	{
		Name:   fmt.Sprintf("%s_addon_kubedashboard_sum", metricShootsCustomPrefix),
